...
while gioco = 1:
    ...
    if scala_colore_g or scala_colore_a:
        ...
        if max(valore_tris_g) != max(valore_tris_a):
            vittoria(full_g, full_a, max(valore_tris_g), max(valore_tris_a))
        elif max(valore_coppia_g) != max(valore_tris_a):
            vittoria(full_g, full_a, max(valore_coppia_g), max(valore_coppia_a))
        ...
        vittoria(colore_g, colore_a, carta_alta_g, carta_alta_a)
    elif scala_g or scala_a:
        vittoria(scala_g, scala_a, inizio_scala_g, inizio_scala_a)
    elif doppia_coppia_g or doppia_coppia_a:
        if max(valore_coppia_g) != max(valore_coppia_a):
            vittoria(doppia_coppia_g, doppia_coppia_a, max(valore_coppia_g), max(valore_coppia_a))
        elif min(valore_coppia_g) != min(valore_coppia_a):
            vittoria(doppia_coppia_g, doppia_coppia_a, min(valore_coppia_g), min(valore_coppia_a))
        else:
            vittoria(doppia_coppia_g, doppia_coppia_a, carta_alta_g, carta_alta_a)
    elif tris_g or tris_a:
        if max(valore_tris_g) != max(valore_tris_a):
            vittoria(tris_g, tris_a, max(valore_tris_g), max(valore_tris_a))
        else:
            vittoria(tris_g, tris_a, carta_alta_g, carta_alta_a)
    elif coppia_g or coppia_a:
        if max(valore_coppia_g) != max(valore_coppia_a):
            vittoria(coppia_g, coppia_a, max(valore_coppia_g), max(valore_coppia_a))
        else:
            vittoria(coppia_g, coppia_a, carta_alta_g, carta_alta_a)
    else:
    ...
    gioco = input("vuoi fare un'altra mano (0 = no, altro tasto = sì)? ")
